async function () {
  ...
  /*
   * Description: Add Loyalty Points For A User Via REST.
  ...
  try {
    ...
    result = {
      ...
      platform,
      username,
      error: { code: "DISCORD_ADD_LOYALTY_ERROR", message },
      ts: new Date().toISOString(),
      data: { runId: RUN_ID }
    };
  }

  try { logDbg(`[DISCORD-ADD-LOYALTY:${RUN_ID}] ${JSON.stringify(result)}`); } catch (_) {}
  finish();
}
