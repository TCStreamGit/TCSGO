async function () {
  ...
  const ACK_TYPE_SELL_START = "sell-start-result";
  const ACK_TYPE_SELL_ALL_START = "sell-all-start-result";
  const ACK_TYPE_SELL_ALL_CONFIRM = "sell-all-confirm-result";
  const ACK_VARS_SELL_START = ["tcsgo_last_sell_start_json", "tcsgo_last_event_json"];
  const ACK_VARS_SELL_ALL_START = ["tcsgo_last_sell_all_start_json", "tcsgo_last_event_json"];
  const ACK_VARS_SELL_ALL_CONFIRM = ["tcsgo_last_sell_all_confirm_json", "tcsgo_last_event_json"];
  ...
  const DEDUPE_STORE_KEY = "tcsgo_sell_chat_dedupe_v2";
  const COOLDOWN_STORE_KEY = "tcsgo_sell_cooldowns_v1";
  const QUEUE_STORE_KEY = "tcsgo_sell_queue_v1";
  ...
  const SELL_ALL_KEY = normalizeCmdKey("sellall");
  const ALL_KEY = normalizeCmdKey("all");
  ...
  async function reply(site, message) {
    ...
    if (!sent) logMsg(`[SELL-CHAT] Reply failed | site=${site} | msg="${msg.slice(0, 120)}"`);
  }
  ...
  async function resolveLinkedIdentity(platformRaw, usernameRaw) {
    ...
    try {
      ...
      logMsg(`[SELL-CHAT] Discord link resolve failed | ${err?.message || err}`);
      return {
        requestedPlatform,
        requestedUsername,
        effectivePlatform: requestedPlatform,
  ...
  async function restAddLoyaltyPoints(value, username, platform) {
    ...
    try {
      ...
      logMsg(`[SELL-CHAT] REST add-loyalty-points failed | ${err?.message || err}`);
  ...
  async function adjustCoins(delta, username, platform, pointsBefore) {
    ...
    if (typeof addLoyaltyPoints === "function") {
      try {
        ...
        logMsg(`[SELL-CHAT] native addLoyaltyPoints failed | ${err?.message || err}`);
      }
    }
  ...
  function ackMatchesJob(payload, job) {
    ...
    if (lowerTrim(payload.type) !== lowerTrim(job.ackType)) return false;
  ...
  async function pollAck(job) {
    ...
    while (Date.now() < deadline) {
      for (const varName of job.ackVars) {
  ...
  async function processSingleJob(job, runnerId) {
    ...
    try {
      if (action === "sell-start") {
        await callCommand({
          name: COMMIT_SELL_START,
          variableValues: {
            eventId,
      ...
      } else if (action === "sell-all-start") {
        await callCommand({
          name: COMMIT_SELL_ALL_START,
          variableValues: {
            eventId,
            platform: effectivePlatform,
            username: effectiveUsername
      ...
      } else if (action === "sell-all-confirm") {
        await callCommand({
          name: COMMIT_SELL_ALL_CONFIRM,
          variableValues: {
            eventId,
            platform: effectivePlatform,
            username: effectiveUsername,
        ...
        logMsg(`[SELL-CHAT] Unknown action | ${action}`);
        return;
      }
    } catch (err) {
      const msg = err?.message ? String(err.message) : String(err);
      logMsg(`[SELL-CHAT] Dispatch failed | action=${action} | ${msg}`);
      await reply(site, `@${usernameRaw} Sell failed to dispatch. Try again.`);
      return;
    }
    ...
    if (!ack) {
      await reply(site, `@${usernameRaw} Sell dispatched. If it does not appear, try again.`);
      return;
    }

    if (!ack.ok) {
      const errMsg = ack?.error?.message || "Sell failed.";
    ...
    if (action === "sell-start") {
      ...
      await reply(
        site,
        `@${usernameRaw} Selling ${d.item?.displayName || "item"} for ${formatNumber(d.creditAmount)} coins ` +
        `(${d.marketFeePercent}% fee). Confirm: ${COMMAND_PREFIX}${SELL_CONFIRM_PRIMARY} ${d.token} (${d.expiresInSeconds}s).`
      );
      return;
    }

    if (action === "sell-all-start") {
      const d = ack.data || {};
      await reply(
        site,
        `@${usernameRaw} Sell-all ready: ${d.eligibleCount} eligible, ${d.lockedCount} locked. ` +
        `Credit: ${formatNumber(d.creditAmount)}. Confirm: ${COMMAND_PREFIX}${COMMAND_PRIMARY} all ${d.token} (${d.expiresInSeconds}s).`
      );
      return;
    }

    if (action === "sell-all-confirm") {
      const d = ack.data || {};
      ...
      if (credit > 0) {
        ...
        if (!creditResult.ok) {
          logMsg(`[SELL-CHAT] Coin credit failed after sell-all confirm | credit=${credit}`);
        }
      }
      await reply(
        site,
        `@${usernameRaw} Sold ${d.soldCount} items. +${formatNumber(d.creditedCoins)} coins.`
      );
    }
  }
  ...
  if (await checkMessageDedupe(site, username, messageId)) {
    logMsg(`[SELL-CHAT] Dedupe | site=${site} | user=${username} | messageId=${messageId}`);
  ...
  if (overlayHandled) {
    logMsg(`[SELL-CHAT] Overlay handled | site=${site} | user=${username}`);
    done({ shouldStop: true });
    return;
  }
  ...
  if (identity.linkedFromDiscord) {
    logMsg(
      `[SELL-CHAT] Discord mapped | ${identity.requestedPlatform}:${identity.requestedUsername} -> ` +
  ...
  const arg0 = String(parsed.args[0] || "").trim();
  if (!arg0) {
    await reply(
      site,
      `@${usernameRaw} Usage: ${COMMAND_PREFIX}${COMMAND_PRIMARY} <oid|itemId|itemName> | ` +
      `${COMMAND_PREFIX}${COMMAND_PRIMARY} all [token]`
    );
    done({ shouldStop: true });
    return;
  }

  const parsedKey = parsed.commandKey;
  const arg0Key = normalizeCmdKey(arg0);

  let action = "sell-start";
  let token = "";
  let oid = "";
  let query = "";

  const sellAllByCommand = parsedKey === SELL_ALL_KEY;
  const sellAllByArg = arg0Key === ALL_KEY;
  const isSellAll = sellAllByCommand || sellAllByArg;

  if (isSellAll) {
    if (sellAllByCommand) {
      token = String(parsed.args[0] || "").trim();
    } else {
      token = String(parsed.args[1] || "").trim();
    }
    action = token ? "sell-all-confirm" : "sell-all-start";
  } else {
    const isOid = arg0.toLowerCase().startsWith("oid_");
    oid = isOid ? arg0 : "";
    query = isOid ? "" : arg0;
  }
  ...
  if (action === "sell-start") {
    await enqueueJob({
      ...
      oid,
      query,
      ackType: ACK_TYPE_SELL_START,
      ackVars: ACK_VARS_SELL_START
    });
  } else if (action === "sell-all-start") {
    await enqueueJob({
      ...jobBase,
      action,
      ackType: ACK_TYPE_SELL_ALL_START,
      ackVars: ACK_VARS_SELL_ALL_START
    });
  } else if (action === "sell-all-confirm") {
    await enqueueJob({
      ...jobBase,
      action,
      token,
      ackType: ACK_TYPE_SELL_ALL_CONFIRM,
      ackVars: ACK_VARS_SELL_ALL_CONFIRM
    });
  }
