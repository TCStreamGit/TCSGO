async function () {
  ...
  const CTX_STORE_KEY = "tcsgo_link_ctx_v1";
  ...
  function parseArgs(rawMessage, message) {
    ...
    // If Message Still Includes The Trigger (Example: "!link"), Strip It
  ...
  async function safeWriteFile(path, content, append = false) {
    ...
    try {
      ...
      try {
        ...
        logExec(`[LINK:${RUN_ID}] WriteFile Error | path=${path} | ${e2?.message ?? e2}`);
        return false;
      }
    }
  }
  ...
  async function replyToCaller(site, message) {
    ...
    if (site === "tiktok") {
      ...
      if (!ok) logExec(`[LINK:${RUN_ID}] TikTok Send Failed | msg="${msg.slice(0, 120)}"`);
      return;
    }
    ...
    logExec(`[LINK:${RUN_ID}] chatbot failed | site=${site} | msg="${msg.slice(0, 120)}"`);
  }
  ...
  const platformDisplayName = String(ctx.displayname || ctx.displayName || "").trim();
  ...
  if (CHAT_DEBUG_ENABLED) {
    try {
      await chatbot({
        site: "twitch",
        message:
          `DEBUG !link | run=${RUN_ID} | username="${platformUsername}" | platform="${platform}" | message="${message}" | rawMessage="${rawMessage}"`
      });
    } catch (_) {}
  }

  logExec(
    `[LINK:${RUN_ID}] Vars | platform=${platform} | username=${platformUsername} | target=${targetDiscordUsername} | msgId=${messageId}`
  );
  ...
  try {
    ...
    if (!platformUsernameLower) {
      throw mkError(
        "MISSING_USERNAME",
        "Missing Username (LINK.JS V2). If This Is Called From Another Command, Pass username/message/rawMessage/platform Into This Command."
      );
    }
    if (!targetLower) {
      throw mkError(
        "MISSING_TARGET",
        "Missing Target Discord Username. Usage: !link <DiscordUsername>"
      );
    }
    ...
    let pendingUnlinkCode = null;
    let pendingUnlinkSession = null;
    ...
    for (const [code, session] of Object.entries(sessions)) {
      ...
      if (sessionAction !== "link") {
        if (!pendingUnlinkSession) {
          pendingUnlinkCode = code;
          pendingUnlinkSession = session;
        }
        continue;
      }

      // Only reuse an existing link if it's from the same platform + username
    ...
    if (pendingUnlinkSession) {
      const pendingPlatform = normSite(pendingUnlinkSession.platform || "");
      const pendingUser = lowerTrim(pendingUnlinkSession.platformUsernameLower || "");
      if (pendingPlatform === platform && pendingUser === platformUsernameLower && pendingUnlinkCode) {
        ok = false;
        error = {
          code: "PENDING_UNLINK",
          message: `An Unlink Is Already Pending For Discord ${targetDiscordUsername}. Use Code ${pendingUnlinkCode} In ${formatDiscordChannel()} Or Wait For It To Expire.`
        };
      } else {
        ok = false;
        error = {
  ...
  try { logDbg(`[LINK:${RUN_ID}] ${JSON.stringify(result)}`); } catch (_) {}
