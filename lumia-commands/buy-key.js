async function () {
  ...
  const STORE_KEY = "tcsgo_buy_key_chat_v2";
  const COOLDOWN_STORE_KEY = "tcsgo_buy_key_cooldowns_v1";
  const QUEUE_STORE_KEY = "tcsgo_buy_key_queue_v1";
  ...
  async function reply(site, message) {
    ...
    if (!sent) logMsg(`[BUY-KEY] Reply failed | site=${site} | msg="${msg.slice(0, 120)}"`);
  }
  ...
  async function resolveLinkedIdentity(platformRaw, usernameRaw) {
    ...
    try {
      ...
      logMsg(`[BUY-KEY] Discord link resolve failed | ${err?.message || err}`);
      return {
        requestedPlatform,
        requestedUsername,
        effectivePlatform: requestedPlatform,
  ...
  function getKeyPriceCoins(prices) {
    try {
      const cadPrice = Number(prices?.keys?.default);
  ...
  async function restAddLoyaltyPoints(value, username, platform) {
    ...
    try {
      ...
      logMsg(`[BUY-KEY] REST add-loyalty-points failed | ${err?.message || err}`);
  ...
  async function pollAck(eventId) {
    ...
    while (Date.now() < deadline) {
      for (const varName of ACK_VARS) {
        ...
        if (payloadEventId === eventId && payloadType === "buykey-result") {
          return payload;
        }
      }
  ...
  async function processSingleJob(job, runnerId) {
    ...
    if (totalCost > 0) {
      ...
      logMsg(`[BUY-KEY] Coins adjusted | method=${adjust.method} | delta=-${totalCost}`);
    }

    try {
      await callCommand({
        name: COMMIT_BUYKEY_COMMAND,
        variableValues: {
          eventId,
      ...
      logMsg(
        `[BUY-KEY] Dispatched | eventId=${eventId} | site=${site} | user=${username} | ` +
        `effective=${effectivePlatform}:${effectiveUsername} | qty=${qty}`
      );
    } catch (err) {
      const msg = err?.message ? String(err.message) : String(err);
      logMsg(`[BUY-KEY] Dispatch failed | ${msg}`);
    ...
    if (ack.ok) {
      ...
      await reply(site, `@${usernameRaw} Bought ${qty}x Key(s).${balanceLabel}`);
      return;
    }
  ...
  if (await checkMessageDedupe(site, username, messageId)) {
    logMsg(`[BUY-KEY] Dedupe | site=${site} | user=${username} | messageId=${messageId}`);
  ...
  if (overlayHandled) {
    logMsg(`[BUY-KEY] Overlay handled | site=${site} | user=${username}`);
    done({ shouldStop: true });
    return;
  }

  const qtyRaw = parsed.args[0];
  const qty = Math.max(1, parseInt(String(qtyRaw ?? "1"), 10) || 1);

  const prices = await loadPrices();
  if (!prices) {
    logMsg("[BUY-KEY] prices.json missing; using fallback key price.");
  }

  const pricePerKey = getKeyPriceCoins(prices);
  ...
  if (identity.linkedFromDiscord) {
    logMsg(
      `[BUY-KEY] Discord mapped | ${identity.requestedPlatform}:${identity.requestedUsername} -> ` +
  ...
  if (!sameIdentity && Number.isFinite(pointsParsed)) {
    logMsg("[BUY-KEY] Points precheck uses caller identity; linked effective identity differs.");
  }
