async function () {
  ...
  log(`[BUYCASE] Vars | eventId=${eventId} | platform=${platform} | username=${username} | alias=${alias} | qty=${qty}`);
  ...
  try {
    ...
    log(`[BUYCASE] Success | ${username} Bought ${qty}x ${displayName} | NewCount=${next}`);
    ...
    log(`[BUYCASE] Error | ${e.code} - ${e.message}`);
  }
